    ...
    creatingButtons(){
        for (let i = 65; i < 91; i++) {
            ...
            button.addEventListener("click", () => {
                let charArray = this.secretWord.split("");
...
//initially hide letters, clear previous word
    ...
    //choose random word
    this.secretWord = optionArray[Math.floor(Math.random() * optionArray.length)];
    this.secretWord = this.secretWord.toUpperCase();

    //replace every letter with span containing dash
    let displayItem = this.secretWord.replace(/./g, '<span class="dashes">_</span>');
